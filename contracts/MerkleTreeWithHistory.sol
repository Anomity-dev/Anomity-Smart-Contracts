...
@dev Hash 2 tree leaves, returns MiMC(_left, _right)
  */
    ...
    function testHash(
        ...
        return hashLeftRight(hasher, _left, _right);
    }
    ...
    function _insertBulk(bytes32[] memory _leaves) internal {
        ...
        for (uint256 i = 0; i < levels; i++) {
            filledSubtrees[i] = filledSubtreesTemp[i];
        }
        nextIndex += uint32(_leaves.length);
        currentRoot = root;
        roots[currentRoot] = true;
    }
    ...
    function _insertLeafInTemp(bytes32 _leaf, bytes32[] memory filledSubtreesTemp, uint256 currentIndex) internal view returns (bytes32) {
        ...
        for (uint32 i = 0; i < levels; i++) {
            if (currentIndex % 2 == 0) {
                ...
                right = zeros(i);
                filledSubtreesTemp[i] = currentLevelHash;
            } else {
                left = filledSubtreesTemp[i];
